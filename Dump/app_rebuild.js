...
function markProjectCompletedForLicense() {
  try {
    ...
    // increment projectsUsed for PRO_STANDARD and for per-license counting
    if (lic.type === 'PRO_STANDARD' || lic.type === 'PRO_MONTHLY' || lic.type === 'PRO_YEARLY' || lic.type === 'ENTERPRISE_YEARLY' || lic.type === 'TRIAL') {
      lic.projectsUsed = (lic.projectsUsed || 0) + 1;
      saveLicense(lic);
      updateLicenseStatus();
      // persist a lightweight project log for tamper detection
      appendProjectLog({ id: window.__lastQuoteMeta?.id || `proj-${Date.now()}`, ts: Date.now(), licenseKey: lic.licenseKey });
      // enforce expiry if needed
      enforceLicenseLimits(lic);
    }
...
function checkLicensePrivilege(action) {
  ...
  if (lic.type === 'PRO_STANDARD') {
    // allow full access until projectsUsed reaches limit
    if ((lic.projectsUsed || 0) >= (LICENSE_TYPES.PRO_STANDARD.expiryProjects || 5)) return false;
    return true;
  }
  // monthly/yearly/enterprise: full access (enterprise seat management not implemented offline)
  return true;
}
